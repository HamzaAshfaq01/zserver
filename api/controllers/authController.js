...
export const preSignup = async (req, res) => {
  try {
    ...
    if (!userName) {
      return res.json({
        error:
          "Please enter a Valid refrelUserName No User Find with this refrelUserName",
      });
    }
    if (!first_name) {
      return res.json({
        error: "Please enter a first_name",
      });
    }
    if (!last_name) {
      return res.json({
        error: "Please enter a last_name",
      });
    }
    if (!email) {
      return res.json({
    ...
    if (!validator.validate(email)) {
      return res.json({
        error: "Please enter a correct email  spelling",
      });
    }
    if (!country) {
      return res.json({
    ...
    if (checkUser) {
      return res.json({
        error: "Email is already in use Choose another Email",
      });
    }
    const checkPhoneNumber = await Auth.findOne({ phone_number });
    if (checkPhoneNumber) {
      return res.json({
        error: "Phone Number is already in use Choose another Phone Number",
      });
    }
    console.log("object");
...
export const signup = async (req, res) => {
  ...
  if (!code || code.length <= 4 || code.length >= 6) {
    return res.json({
      error: "Please enter a valid code to activate your account",
    });
  }
...
if(!check){
  return res.json({
    error:"Please Enter Valid UerName"
  })
}
if(!title){
  return res.json({
...
export const fetchUserProfile = async (req, res) => {
  ...
  const addamount1 = user?.totalAmount * 0.005;
...
export const depositUser = async (req, res) => {
  ...
  if (!image) {
    return res.json({ error: "Please provide an image to deposit" });
  }

  if (!amount) {
    return res.json({ error: "Please provide an amount to deposit" });
  }
  if (!accountId) {
...
export const ApprovedepositUser = async (req, res) => {
  ...
  const commision = user.investedAmount * 0.09;
  ...
  const firstHeadsofUserHead = (await user.investedAmount) * 0.04;
  const secondHeadsofUserHead = (await user.investedAmount) * 0.03;
  const thirdHeadsofUserHead = (await user.investedAmount) * 0.02;
  const fourthHeadsofUserHead = (await user.investedAmount) * 0.02;
